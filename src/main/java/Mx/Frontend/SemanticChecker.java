...
public class SemanticChecker implements ASTVisitor {
    ...
    private final Stack<StmtNode> loopStack;
        ...
        if (node.getDeclarations()!=null) {
            for (var decl: node.getDeclarations()) {
                if (decl instanceof FuncNode) {
                    try {
                        if (typeTable.hasType(((FuncNode)decl).getPureName())) {
            ...
            FunctionEntity mainFunc = currentScope().getOverloadedFuncEntities(
                    "main").get(0);
            if (mainFunc==null) {
                ...
                if (!(typeTable.getType(mainFunc.getReturnType()
                        ,exceptionHandler).equals(new IntType()) ) ) {
    ...
    public void visit(FuncCallExprNode node) {
        ...
        if (funcName instanceof MemberExprNode) {
            ...
            if (objectType instanceof ArrayType) {
                ...
                func = ((ClassType) objectType).getMethod(methodName, parameters);
            }
            else { // String method
            ...
            for (var f: currentScope().getOverloadedFuncEntities(
                    ((IdExprNode) funcName).getIdentifier() ) ) {
                if (FuncNameDecorator.funcCallMatched(f.getName(), decoratedEnd)) {
                    found = true;
                    func = f;
                    break;
                }
    ...
    public void visit(MemberExprNode node) {
        ...
        // "name" here is the name that hasn't been decorated (if it's a name of function)
    ...
    public void visit(AssignmentExprNode node) {
        ...
        if (Type.canNotAssign(lExpr.getType(), rExpr.getType())) {
    ...
    public void visit(ReturnStmtNode node) {
        ...
        if (retSpecifier!=null) {
            ...
            if (node.hasReturnValue()) {
                ...
                if (Type.canNotAssign(retType, stmtRetType)) {
    ...
    public void visit(ClassSpecifierNode node) {
        ...
        if (node.hasConstructor()) {
            ...
            for (var constructor: constructors) {
                if (!constructor.getPureName().equals(node.getClassName())) {
                ...
                try {
                    scope.declareFunction(constructor,
                            FunctionEntity.FuncEntityType.constructor,
                            node.getClassName(), exceptionHandler);
                }
                catch (SemanticError err) {
                    errored = true;
                }
        ...
        if (methods!=null) {
            for (var method: methods) {
                if (method.getPureName().equals(node.getClassName())) {
                ...
                try {
                    scope.declareFunction(method,
                            FunctionEntity.FuncEntityType.method,
                            node.getClassName(), exceptionHandler);
                }
                catch (SemanticError err) {
                    errored = true;
        ...
        if (errored) throw new SemanticError();
    }
    ...
    public void visit(FuncNode node) {
        ...
        FunctionEntity func = currentScope().getFuncEntity(node.getDecoratedName());
        if (func!=null) {
            ArrayList<VarEntity> parameterEntities = func.getParameters();
            ArrayList<VarNode> parameters = node.getParameters();
            if (parameterEntities.size() != parameters.size()) {
                errored = true;
            }
            else {
                for (int i = 0, it = parameters.size(); i < it; ++i) {
